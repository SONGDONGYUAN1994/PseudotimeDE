...
Although we do not encourage this, users may choose completely ignore the uncertainty of inferred pseudotime, and use the $p$-values based on an asymptotic distribution (`fix.pv`). Note that `fix.pv` usually do not behave correctly under the null (i.g., following a $\operatorname{Uniform}(0, 1)$ distribution). The good thing is that only calculating `fix.pv` makes the DE test very fast.
